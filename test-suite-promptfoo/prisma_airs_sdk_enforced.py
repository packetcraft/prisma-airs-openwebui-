...
class Filter:
    class Valves(BaseModel):
        ...
        AI_PROFILE_NAME: str = Field(
            default="ark-sec-profile", description="AI Security Profile name"
        )
    ...
    async def inlet(
            ...
            await __event_emitter__(
                {
                    "type": "status",
                    "data": {
                        "description": "🛡️ Prisma AIRS: Shielding Prompt...",
                        "done": False,
                    },
                }
            )
        ...
        try:
            ...
            result = scanner.sync_scan(
                ai_profile=profile, content=Content(prompt=user_prompt)
            )
            ...
            if result.action == "block":
                    ...
                    await __event_emitter__(
                        {
                            "type": "status",
                            "data": {
                                "description": "🚨 BLOCK: Malicious Prompt",
                                "done": True,
                            },
                        }
                    )
                ...
                raise Exception(
                    f"Security Block: Malicious input detected. (Scan ID: {result.scan_id})"
                )
    ...
    async def outlet(
            ...
            await __event_emitter__(
                {
                    "type": "status",
                    "data": {
                        "description": "🛡️ Prisma AIRS: Sanitizing Response...",
                        "done": False,
                    },
                }
            )
        ...
        try:
            ...
            result = scanner.sync_scan(
                ai_profile=profile, content=Content(prompt=user_prompt, response=ai_res)
            )
            ...
            if result.action == "block" and not has_dlp_hits:
                body["messages"][-1]["content"] = (
                    f"🚨 **Security Block**: Dangerous content detected and removed. (Scan ID: `{result.scan_id}`)"
                )
            ...
            # 2. MASKING: Sensitive data like AWS Keys
                ...
                masked_text = masked_info.get(
                    "masked_response", " [REDACTED BY SECURITY POLICY] "
                )
                body["messages"][-1]["content"] = (
        ...
        if __event_emitter__:
            await __event_emitter__(
                {
                    "type": "status",
                    "data": {"description": f"Prisma {status}", "done": True},
                }
            )
